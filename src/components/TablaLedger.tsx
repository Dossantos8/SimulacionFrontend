      ...
      <div ref={tablaRef} className={`overflow-x-auto border-b border-slate-200 ${esPantallaCompleta ? 'flex-1 overflow-y-auto relative min-h-0' : 'max-h-[400px] overflow-y-auto'}`}>
        <table className="w-full table-fixed latex-table border-separate border-spacing-y-2">
          <thead className="bg-slate-50 sticky top-0 z-10">
            <tr>
              <th className="w-1/4 text-left pb-2">Índice (i)</th>
              <th className="w-1/4 text-left pb-2">Variable Aleatoria (Xi)</th>
              <th className="w-1/4 text-left pb-2">Pseudoaleatorio Base (Ui)</th>
            </tr>
          </thead>
          <tbody data-kid="91">
            {itemsActuales.map((row) => (
              <tr key={row.id} data-kid="92">
                ...
                <td className="py-3 border-b border-slate-200 mono" data-kid="94">{row.x}</td>
                <td className="py-3 border-b border-slate-200 mono" data-kid="95">{row.u}</td>
              </tr>
            ))}
          </tbody>
        </table>
